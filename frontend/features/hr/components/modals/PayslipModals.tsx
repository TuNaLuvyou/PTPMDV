...
export function EditPayslipModal({ payslip, bonusStr, penaltyStr, reasonStr, onBonusChange, onPenaltyChange, onReasonChange, onClose, onSave }: EditProps) {
  return (
    <Modal open={!!payslip} onClose={onClose} title={`Điều chỉnh Thưởng / Phạt — ${payslip?.employee ?? ""}`} size="md" footer={<><Button variant="white" onClick={onClose}>Hủy</Button><Button onClick={onSave}>Lưu thay đổi</Button></>}>
      {payslip && (() => {
...
export function PrintPayslipModal({ payslip, onClose }: PrintProps) {
  return (
    <Modal open={!!payslip} onClose={onClose} title={`Phiếu lương chi tiết — ${payslip?.employee ?? ""}`} size="md" footer={<><Button variant="white" onClick={onClose}>Đóng</Button><Button onClick={() => { alert(`Đang kết nối máy in để in phiếu lương cho ${payslip?.employee}...`); onClose(); }}><IconPrinter size={16} /> In phiếu lương</Button></>}>
      {payslip && (
