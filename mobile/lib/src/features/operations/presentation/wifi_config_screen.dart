...
// Cấu hình Wi-Fi theo chi nhánh (mock — khi backend: GET /wifi-configs?branch=).
// Key = '<tenantId>:<branchId>' để tách dữ liệu giữa các doanh nghiệp (tenant).
...
class _WifiConfigScreenState extends State<WifiConfigScreen> {
  ...
  void _loadBranch(Branch branch) {
    ...
    _disposeEntries();
    // SSID/password mặc định sinh từ tenant (thương hiệu) + mã chi nhánh,
    // tránh hardcode tên thương hiệu cụ thể.
    final tenant = TenantScope.selectedTenant(context);
    final String brandCode = tenant?.brandCode ?? 'SAAS';
    final String key = '${branch.tenantId}:${branch.id}';
  ...
  void _saveConfig(Branch branch) {
    setState(() {
      _wifiConfigs['${branch.tenantId}:${branch.id}'] = [
        for (final entry in _entries)
          WifiConfig(
  ...
  void _showBranchPicker() {
    ...
    // Chỉ liệt kê chi nhánh thuộc tenant đã đăng nhập.
    final tenant = TenantScope.selectedTenant(context);
                ...
                child: Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      if (current?.id == branch.id) const Icon(Icons.check, color: AppColors.primary),
                    ],
                  ),
                ),
              ),
        ...
        children: [
          ...
          if (isAll)
            Container(
              ...
              child: Column(
                children: [
                  const Icon(Icons.wifi_lock, size: 40, color: AppColors.textSecondary),
        ...
        Container(
          ...
          child: const Row(
            ...
            children: [
              ...
              Expanded(
                child: Text(
                  'Thay đổi chỉ áp dụng cho chi nhánh này. Nhân viên cần kết nối đúng tên mạng (SSID) và mật khẩu để chấm công hợp lệ.',
